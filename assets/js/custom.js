...
function toggleReadMore(event, index) {
  ...
  const dots = document.querySelectorAll(".dots")[index - 1];
  const moreText = document.querySelectorAll(".more")[index - 1];
  const btn = event.target;

  if (dots.style.display === "none") {
    dots.style.display = "inline";
    moreText.style.display = "none";
    moreText.style.padding = "0";
    moreText.style.margin = "0";
...
document.querySelectorAll(".read-more-btn").forEach(function (button) {
  button.addEventListener("click", function () {
    const parent = this.closest(".home1-testm-txt");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const elements = document.querySelectorAll(".packages-content .col-md-4");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  accordionButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      // Jika item sudah aktif, tutup
      if (accordionItem.classList.contains("active")) {
        accordionItem.classList.remove("active");
      } else {
        // Tutup semua accordion items
